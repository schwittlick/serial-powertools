#!/usr/bin/env python3
"""
progress_server.py — run this on whichever host should hold job state.
Listens on TCP port 9876 (override via PROGRESS_BIND_HOST / PROGRESS_PORT
environment variables) for job reporters, serves current state to waybar
polling clients.
...
HOST = os.environ.get("PROGRESS_BIND_HOST", "0.0.0.0")
...
# callbacks to notify when a job finishes (for notify-send on the desktop side,
